...
public class MetaModel<T> { // We're inferring that the MetaModel Class can only be a metamodel of another
	...
	// Let's create a method to check and then transpose a normal java class to a
	// MetaModel Class
	...
	public List<ColumnField> getColumns() {
		...
		// This method return all the properties of the class that are marked with
		// @Column

		if (columnFields.size() == 0) {
			Field[] fields = clazz.getDeclaredFields();

			// for each field within the above field[] check to see if it has a column
			// annotation and if it does,
			...
			// Let's just add some extra logic in the case that the entity doesn't have any
			// column fields
	...
	public List<ColumnField> getNonPKColumns() {

		// This method return all the properties of the class that are marked with
		// @Column AND are NOT primary keys
		Field[] fields = clazz.getDeclaredFields();
		List<ColumnField> retList = new ArrayList<>();

		for (Field field : fields) {
			if (field.getAnnotation(Id.class) == null) {
				retList.add(new ColumnField(field));
			}
		}
	...
	// Let's do the foreign key field first since this should match almost exactly
	// to our getColumnFields
	...
	public List<ForeignKeyField> getForeignKeys() {
		...
		for (Field field : fields) {
			...
			// Create a column object, this will not be null if the field is annotated with
			// @Column
	...
	public PrimaryKeyField getPrimaryKey() {
		...
		for (Field field : fields) {
			...
			// If the primary key is not null then generate a primary key field object from
			// that field
			if (primaryKey != null) {
